...
function mergeWithDeep(target, source, merge2, stack) {
  ...
  for (let i = 0; i < sourceKeys.length; i++) {
    ...
    const merged = merge2(targetValue, sourceValue, key2, target, source, stack);
    if (merged !== void 0) {
      target[key2] = merged;
    } else if (Array.isArray(sourceValue)) {
      target[key2] = mergeWithDeep(targetValue, sourceValue, merge2, stack);
    } else if (isObjectLike(targetValue) && isObjectLike(sourceValue)) {
      target[key2] = mergeWithDeep(targetValue, sourceValue, merge2, stack);
    } else if (targetValue == null && isPlainObject(sourceValue)) {
      target[key2] = mergeWithDeep({}, sourceValue, merge2, stack);
  ...
  const vueApp = await Promise.all([
    ...
    return setup({
      ...
      props: {
        ...
        resolveComponent,
        titleCallback: title,
        onHeadUpdate: isServer ? (elements) => head3 = elements : null
      },
      plugin
    });
  });
